...
export class ReqService {
  ...
  /*规则是，如果用户在10分钟类没有请求任何数据，则登录账号失效*/
  private countValidator: Number = 600;
  private loginTime;
  private interval;
  ...
  // 每次请求都要刷新计时表，判断账号是否有效
  public refreshCountValidator(): void {
    if (this.countValidator <= 600) {
  ...
  public setLoginTime(loginTime: any): void {
    this.loginTime = loginTime;
  }
  public setInterval(interval: any): void {
  ...
  public setCountValidator(countValidator: Number): void {
        if (this.countValidator > 600) {
          this.cleanInterval();
          this.router.navigate(['/login']);
        }
        console.log(this.countValidator);
        this.countValidator = countValidator;
  }
  public getCountValidator(): Number {
